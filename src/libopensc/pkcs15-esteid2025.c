...
sc_pkcs15emu_esteid2025_init(sc_pkcs15_card_t *p15card)
{
	...
	if (is_fin_eid(p15card)) {
		...
		if (p15card->tokeninfo->serial_number == NULL)
			LOG_TEST_RET(card->ctx, SC_ERROR_OUT_OF_MEMORY, "Allocating memory failed");
