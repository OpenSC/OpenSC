...
pgp_decode_kdf_do(sc_card_t *card, struct pgp_priv_data *priv)
{
	...
	const uint8_t *const data = kdf_blob->data;
	const size_t data_len = kdf_blob->len;
	size_t tag_len;
	const uint8_t *p = sc_asn1_find_tag(card->ctx, data, data_len, 0x81, &tag_len);
	if (!p) {
...
kdf_itersalted_s2k(struct sc_context *ctx, const char *hash, const uint8_t *pin, size_t pinlen, const uint8_t *salt, size_t saltlen, uint32_t iterations, uint8_t **out, size_t *outlen)
{
	...
	const int keysize = EVP_MD_size(md);
	if (keysize <= 0) {
		goto out2;
	}
	uint8_t *outkey = (uint8_t *)sc_mem_secure_alloc(keysize);
	if (outkey == NULL) {
