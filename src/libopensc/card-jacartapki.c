...
jacartapki_fcp_encode(struct sc_card *card, const struct sc_file *file, unsigned char **out)
{
	...
	buf_size = 13 + (file->namelen > 0 ? file->namelen + 2 : 0) + (file->prop_attr != NULL && file->prop_attr_len > 0 ? file->prop_attr_len + 2 : 0) + ops_len * 2 + (file->encoded_content != NULL ? file->encoded_content_len : 0);
		...
		unsigned char **out, size_t *out_len)
{
	...
	if (EVP_DecryptInit_ex2(cctx, alg, key, icv, NULL) == 0) {
		sc_log_openssl(ctx);
	...
	if (EVP_DecryptUpdate(cctx, decrypted, &tmplen, data, (int)data_len) == 0) {
		sc_log_openssl(ctx);
		EVP_CIPHER_CTX_free(cctx);
		sc_evp_cipher_free(alg);
		free(decrypted);
	...
	if (EVP_DecryptFinal_ex(cctx, decrypted + decrypted_len, &tmplen) == 0) {
		sc_log_openssl(ctx);
		EVP_CIPHER_CTX_free(cctx);
		sc_evp_cipher_free(alg);
		free(decrypted);
		...
		unsigned char **out, size_t *out_len, int not_force_pad)
{
	...
	*out = calloc(data_len + 8, sizeof(unsigned char));
	...
	cctx = EVP_CIPHER_CTX_new();
	alg = sc_evp_cipher(ctx, "DES-EDE-CBC");
	if (EVP_EncryptInit_ex2(cctx, alg, key, icv, NULL) == 0) {
		sc_log_openssl(ctx);
		free(*out);
		free(data);
		EVP_CIPHER_CTX_free(cctx);
	...
	/* Disable padding, otherwise it will fail to decrypt non-padded inputs */
	EVP_CIPHER_CTX_set_padding(cctx, 0);
	if (EVP_EncryptUpdate(cctx, *out, &tmplen, data, (int)data_len) == 0) {
		sc_log_openssl(ctx);
		free(*out);
		free(data);
		EVP_CIPHER_CTX_free(cctx);
	...
	*out_len = tmplen;
	...
	if (EVP_EncryptFinal_ex(cctx, *out + *out_len, &tmplen) == 0) {
		sc_log_openssl(ctx);
		free(*out);
		free(data);
		EVP_CIPHER_CTX_free(cctx);
