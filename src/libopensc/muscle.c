...
int msc_list_objects(sc_card_t* card, u8 next, mscfs_file_t* file) {
	...
	if(apdu.resplen == 0) /* No more left */
		return 0;
	if (apdu.resplen != 14) {
		sc_log(card->ctx, "expected 14 bytes, got %" SC_FORMAT_LEN_SIZE_T "u.\n", apdu.resplen);
...
int msc_partial_read_object(sc_card_t *card, msc_id objectId, int offset, u8 *data, size_t dataLength)
{
	...
	sc_log(card->ctx, "READ: Offset: %x\tLength: %" SC_FORMAT_LEN_SIZE_T "u\n", offset, dataLength);
...
int
msc_read_object(sc_card_t *card, msc_id objectId, int offset, u8 *data, size_t dataLength)
{
	int r;
	size_t i;
...
int
msc_zero_object(sc_card_t *card, msc_id objectId, size_t dataLength)
{
...
int
msc_create_object(sc_card_t *card, msc_id objectId, size_t objectSize, unsigned short readAcl, unsigned short writeAcl, unsigned short deleteAcl)
{
	u8 buffer[14];
	sc_apdu_t apdu;
	int r;
...
int msc_partial_update_object(sc_card_t *card, msc_id objectId, int offset, const u8 *data, size_t dataLength)
{
	...
	if (card->ctx->debug >= 2)
		sc_log(card->ctx, "WRITE: Offset: %x\tLength: %" SC_FORMAT_LEN_SIZE_T "u\n", offset, dataLength);
...
int
msc_verify_pin_apdu(sc_card_t *card, sc_apdu_t *apdu, u8 *buffer, size_t bufferLength, int pinNumber, const u8 *pinValue, int pinLength)
{
...
int
msc_unblock_pin(sc_card_t *card, int pinNumber, const u8 *pukValue, int pukLength, int *tries)
{
	sc_apdu_t apdu;
	int r;
	const int bufferLength = MSC_MAX_PIN_LENGTH;
...
int
msc_unblock_pin_apdu(sc_card_t *card, sc_apdu_t *apdu, u8 *buffer, size_t bufferLength, int pinNumber, const u8 *pukValue, int pukLength)
{
...
int
msc_change_pin(sc_card_t *card, int pinNumber, const u8 *pinValue, int pinLength, const u8 *newPin, int newPinLength, int *tries)
{
	sc_apdu_t apdu;
	int r;
...
/* USE ISO_VERIFY due to tries return */
int
msc_change_pin_apdu(sc_card_t *card, sc_apdu_t *apdu, u8 *buffer, size_t bufferLength, int pinNumber,
		const u8 *pinValue, int pinLength, const u8 *newPin, int newPinLength)
{
	u8 *ptr;
...
int
msc_get_challenge(sc_card_t *card, unsigned short dataLength, unsigned short seedLength, u8 *seedData, u8 *outputData)
{
	sc_apdu_t apdu;
	int r, location, cse;
	size_t len;
...
int
msc_generate_keypair(sc_card_t *card, int privateKey, int publicKey, int algorithm, int keySize, int options)
{
	sc_apdu_t apdu;
...
int
msc_extract_key(sc_card_t *card, int keyLocation)
{
	sc_apdu_t apdu;
	u8 encoding = 0;
	int r;
...
int
msc_extract_rsa_public_key(sc_card_t *card,
		int keyLocation,
		size_t *modLength,
		u8 **modulus,
		size_t *expLength,
...
int
msc_compute_crypt_init(sc_card_t *card,
		int keyLocation,
		int cipherMode,
		int cipherDirection,
		const u8 *initData,
...
int
msc_compute_crypt_final(sc_card_t *card,
		int keyLocation,
		const u8 *inputData,
		u8 *outputData,
		size_t dataLength,
...
static int
msc_compute_crypt_final_object(sc_card_t *card,
		int keyLocation,
		const u8 *inputData,
		u8 *outputData,
		size_t dataLength,
...
int
msc_compute_crypt(sc_card_t *card,
		int keyLocation,
		int cipherMode,
		int cipherDirection,
		const u8 *data,
