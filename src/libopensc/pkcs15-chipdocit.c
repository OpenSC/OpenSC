...
static int read_ds_cert(sc_pkcs15_card_t *p15card, unsigned fid,
	const sc_pkcs15_id_t *cert_id, const char *label, int user_consent_hint)
{
	...
	apdu.resp = rbuf; apdu.resplen = sizeof rbuf; apdu.le = 256;
	...
	if (raw[0] == 0x30) {			/* plain DER */
		...
		der = malloc(8192);
		if (!der) { free(rot); return SC_ERROR_OUT_OF_MEMORY; }
		memset(&z, 0, sizeof z);
		if (inflateInit2(&z, -15) != Z_OK
		    || inflateSetDictionary(&z, dict, (uInt) dlen) != Z_OK) {
			inflateEnd(&z); free(der); free(rot);
...
static int add_pin(sc_pkcs15_card_t *p15card)
{
...
static int add_prkey(sc_pkcs15_card_t *p15card, const sc_pkcs15_id_t *id,
...
int sc_pkcs15emu_chipdocit_init_ex(sc_pkcs15_card_t *p15card, struct sc_aid *aid)
{
	sc_card_t *card = p15card->card;
	...
	id_auth.len = 1; id_auth.value[0] = 0x01;
	memset(&id_ds, 0, sizeof id_ds);
	id_ds.len = 1; id_ds.value[0] = 0x02;
	u8 csn[32];
	int r;
	...
	apdu.resp = csn; apdu.resplen = sizeof csn; apdu.le = 256;
	if (sc_transmit_apdu(card, &apdu) == SC_SUCCESS
	    && sc_check_sw(card, apdu.sw1, apdu.sw2) == SC_SUCCESS && apdu.resplen) {
