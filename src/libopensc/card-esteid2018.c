...
static int esteid_set_security_env(sc_card_t *card, const sc_security_env_t *env, int se_num) {
	...
	new_env = *env;
	if (env->algorithm == SC_ALGORITHM_EC && env->operation == SC_SEC_OPERATION_SIGN && (env->key_ref[0] & SIGN_REF) > 0) {
		new_env.algorithm_ref = 0x54;
	} else if (env->algorithm == SC_ALGORITHM_EC && env->operation == SC_SEC_OPERATION_SIGN) {
