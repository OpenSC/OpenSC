...
static int cardos_init(sc_card_t *card)
{
	...
	if (card->flags) {
		...
		flags = 0;
		if (card->type == SC_CARD_TYPE_CARDOS_V5_0) {
			flags |= SC_ALGORITHM_RSA_PAD_PKCS1 | SC_ALGORITHM_RSA_PAD_PSS;
		} else if(card->type == SC_CARD_TYPE_CARDOS_V5_3) {
			flags |= SC_ALGORITHM_RSA_RAW | SC_ALGORITHM_RSA_HASH_NONE | SC_ALGORITHM_RSA_PAD_PSS | SC_ALGORITHM_ONBOARD_KEY_GEN;
		} else {
			    ...
			    int se_num)
{
	...
	if (card->type == SC_CARD_TYPE_CARDOS_V5_0 || card->type == SC_CARD_TYPE_CARDOS_V5_3) {
		...
		if (key_id & 0xF0) {
			...
			data[8] = 0x10;
			apdu.lc = apdu.datalen = 9;
		} else if (env->operation == SC_SEC_OPERATION_SIGN && priv->sec_env->algorithm_flags & SC_ALGORITHM_RSA_PAD_PSS) {
			data[6] = 0x80;
			data[7] = 0x01;
