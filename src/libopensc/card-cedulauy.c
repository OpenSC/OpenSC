...
/* failures are silent, the card is just not matched: no usable home directory means no cache, use CEDULAUY_MRZ */
static int
cedulauy_get_mrz(sc_context_t *ctx, unsigned char *mrz)
{
	...
	if (env != NULL) {
		...
		sc_log(ctx, "Ignoring CEDULAUY_MRZ, it is not %d characters long",
				CEDULAUY_MRZ_LEN);
	}

	return cedulauy_read_mrz_cache(ctx, mrz);
}
...
cedulauy_match_contactless(struct sc_card *card)
{
	...
	if (!cedulauy_get_mrz(card->ctx, mrz)) {
