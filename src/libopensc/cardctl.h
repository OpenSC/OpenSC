...
enum {
	...
	/*
	 * JaCarta PKI specific card_ctl calls
	 */
	SC_CARDCTL_JACARTAPKI_BASE = _CTL_PREFIX('J', 'K', 'I'),
	SC_CARDCTL_JACARTAPKI_GENERATE_KEY,
	SC_CARDCTL_JACARTAPKI_UPDATE_KEY,
};
...
typedef struct sc_cardctl_openpgp_key_gen_store_info {
	...
	union {
		...
		struct {
			...
			size_t oidv_len;
			size_t key_length; /* field_length bits */
		} ec;
	} u;
...
#define SC_HSM_MANAGE_KEY_DOMAIN_DELETE_KEY_DOMAIN	4
#define SC_HSM_MANAGE_KEY_DOMAIN_CLEAR_KEK		5
#define SC_HSM_MANAGE_KEY_DOMAIN_ASSOCIATE_KEY_DOMAIN	6
