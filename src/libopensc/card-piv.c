...
static int piv_encode_apdu(sc_card_t *card, sc_apdu_t *plain, sc_apdu_t *sm_apdu)
{
	...
	sm_apdu->p1 = plain->p1;
	sm_apdu->p2 = plain->p2;
	/* all APDU become cse=4  cse=1 stay short. May support extended in future */
	sm_apdu->cse = (plain->cse & ~0x07) + 4;
...
static int piv_decode_apdu(sc_card_t *card, sc_apdu_t *plain, sc_apdu_t *sm_apdu)
{
	...
	/* some commands to not have response data */
	if (ee.value != NULL) {
		p = ee.value;
		inlen = ee.len;
		...
		sc_cardctl_piv_genkey_info_t *keydata)
{
	...
	if (r >= 0) {
		...
		if (r < 0 || cp == NULL || !piv_is_expected_tag(cla_out, tag_out, SC_ASN1_CTX | SC_ASN1_CONS | 0x1F49)) {
...
/*
 * with sp800-73-4 and SM  GUID is also in  sm_cvc. sugjectID 
 */
static int
