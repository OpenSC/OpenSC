...
static int
_jacartapki_type(int id)
{
	if ((id & 0xFF00) == 0x0) {
		if ((id & 0xC0 /*JACARTAPKI_FS_REF_MASK*/) == JACARTAPKI_FS_BASEFID_PUBKEY)
			return JACARTAPKI_TYPE_PUBKEY;
		else
			return JACARTAPKI_TYPE_PRVKEY;
	}

	switch (id & JACARTAPKI_BASEKX_MASK) {
	case 0x0100:
		return JACARTAPKI_TYPE_KX_PUBKEY;
	case 0x0200:
		return JACARTAPKI_TYPE_KX_PRVKEY;
	case 0x0300:
		return JACARTAPKI_TYPE_KX_SKEY;
	case 0x0400:
	case 0x0500:
		return JACARTAPKI_TYPE_KX_CERT;
	case 0x0600:
		return JACARTAPKI_TYPE_KX_DATA;
	}
		...
		char *pin_path, unsigned char auth_id, unsigned flags)
{
	...
	struct jacartapki_ko_props *props = NULL;
	int rv;
	...
	auth_info.auth_method = (auth_id == JACARTAPKI_TRANSPORT_PIN1_AUTH_ID) ? SC_AC_AUT : SC_AC_CHV;
...
int
sc_pkcs15emu_jacartapki_create_pin(struct sc_pkcs15_card *p15card, char *label,
		char *pin_path, unsigned char auth_id, unsigned flags)
{
...
_create_pubkey(struct sc_pkcs15_card *p15card, unsigned file_id)
{
	...
	unsigned ko_fid = ((file_id & JACARTAPKI_FS_REF_MASK) | JACARTAPKI_FS_BASEFID_PUBKEY) + 1;
	struct sc_path path;
	unsigned char fid[2] = {((file_id >> 8) & 0xFF), (file_id & 0xFF)};
	unsigned char *data = NULL;
	size_t len;
...
_create_prvkey(struct sc_pkcs15_card *p15card, unsigned file_id)
{
	...
	struct sc_file *key_file = NULL;
	unsigned ko_fid = ((file_id & JACARTAPKI_FS_REF_MASK) | JACARTAPKI_FS_BASEFID_PRVKEY_EXCH) + 1;
	struct sc_path path;
	unsigned char fid[2] = {((file_id >> 8) & 0xFF), (file_id & 0xFF)};
	unsigned char *data = NULL;
	size_t len;
...
_parse_fs_data(struct sc_pkcs15_card *p15card)
{
	...
	for (df = 0; df_paths[df]; df++) {
		...
		for (ii = 0; ii < count; ii++) {
			...
			switch (type) {
			case JACARTAPKI_TYPE_KX_PRVKEY:
			...
			case JACARTAPKI_TYPE_KX_CERT:
				...
				break;
			case JACARTAPKI_TYPE_KX_PUBKEY:
	...
	for (ii = 0; ii < dobjs_num; ii++) {
		...
		for (offs = 0, rec_num = 0; offs < data->data_len; rec_num++) {
			...
			if (rec->keysize_sign != 0 || rec->keysize_keyexchange != 0) {
				...
				rv = jacartapki_md_cmap_record_guid(ctx, rec, &guid, &guid_len);
				...
				for (ii = 0; ii < prkeys_num; ii++) {
					...
					int key_idx = (info->key_reference & JACARTAPKI_FS_REF_MASK) - JACARTAPKI_FS_KEY_REF_MIN;
...
static int
sc_pkcs15emu_jacartapki_init(struct sc_pkcs15_card *p15card)
{
	struct sc_context *ctx = p15card->card->ctx;
	struct sc_path path;
	unsigned char *buf = NULL;
	...
	rv = _create_pin(p15card, "SO PIN", PATH_SOPIN, JACARTAPKI_SO_PIN_AUTH_ID, SC_PKCS15_PIN_FLAG_SO_PIN);
...
static int
jacartapki_detect_card(sc_pkcs15_card_t *p15card)
{
	struct sc_context *ctx = p15card->card->ctx;
	struct sc_card *card = p15card->card;

	sc_log(ctx, "jacartapki_detect_card (%s)", card->name);
	if (card->type != SC_CARD_TYPE_JACARTA_PKI)
...
static void
jacartapki_clear(struct sc_pkcs15_card *p15card)
{
	struct sc_context *ctx = p15card->card->ctx;
	LOG_FUNC_CALLED(ctx);
}
...
int
sc_pkcs15emu_jacartapki_init_ex(struct sc_pkcs15_card *p15card, struct sc_aid *aid)
{
	struct sc_context *ctx = p15card->card->ctx;
	int rv;
	...
	rv = jacartapki_detect_card(p15card);
	LOG_TEST_RET(ctx, rv, "Not JaCarta PKI token/card");

	/* P15 DF RELOAD PRIVATE */
	p15card->ops.parse_df = jacartapki_parse_df;
	p15card->ops.clear = jacartapki_clear;

	rv = sc_pkcs15emu_jacartapki_init(p15card);
