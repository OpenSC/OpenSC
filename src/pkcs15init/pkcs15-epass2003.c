...
#include "profile.h"
static int epass2003_pkcs15_erase_card(struct sc_profile *profile, struct sc_pkcs15_card *p15card)
{
...
static int epass2003_pkcs15_init_card(struct sc_profile *profile, struct sc_pkcs15_card *p15card)
{
...
static int epass2003_pkcs15_create_dir(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
				       struct sc_file *df)
{
	struct sc_card *card = p15card->card;
	int ret;
	...
	{			/* p15 DF */
		...
		sc_file_free(skey_file);
		 ...
		 SC_TEST_RET(card->ctx,SC_LOG_DEBUG_NORMAL, ret,"Get MAXPIN info failed");
...
static int epass2003_pkcs15_pin_reference(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
...
static int epass2003_pkcs15_create_pin(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
				       ...
				       const unsigned char *puk, size_t puk_len)
{
	...
	{			/*pin */
		...
		data.key_data.es_secret.ac[0] = EPASS2003_AC_MAC_NOLESS|EPASS2003_AC_EVERYONE;
		data.key_data.es_secret.ac[1] = EPASS2003_AC_MAC_NOLESS|EPASS2003_AC_USER;

		r = sc_profile_get_pin_id(profile, 2, &id);
		SC_TEST_RET(card->ctx,SC_LOG_DEBUG_NORMAL, r,"Get User PIN id error!");
		data.key_data.es_secret.EC = sc_profile_get_pin_retries(profile, id);
	...
	{			/*puk */
		...
		data.key_data.es_secret.ac[0] = EPASS2003_AC_MAC_NOLESS|EPASS2003_AC_EVERYONE;
		data.key_data.es_secret.ac[1] = EPASS2003_AC_MAC_EQUAL|EPASS2003_AC_SO;
...
static int epass2003_pkcs15_key_reference(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
...
static int epass2003_pkcs15_create_key(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
...
static int epass2003_pkcs15_store_key(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
							   struct sc_pkcs15_object *obj, struct sc_pkcs15_prkey *key)
{
	struct sc_card *card = p15card->card;
...
static int epass2003_pkcs15_generate_key(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
								  struct sc_pkcs15_object *obj, struct sc_pkcs15_pubkey *pubkey)
{
	struct sc_card *card = p15card->card;
	int r;
...
static int epass2003_pkcs15_delete_object (struct sc_profile *profile, struct sc_pkcs15_card *p15card,
...
static int epass2003_pkcs15_sanity_check(sc_profile_t *profile, sc_pkcs15_card_t *p15card)
{
