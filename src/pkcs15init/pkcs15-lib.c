...
static const char *
find_library(sc_context_t *ctx, const char *name)
{
	int          i;
...
static void *
load_dynamic_driver(sc_context_t *ctx, void **dll,
	const char *name)
{
		...
		struct sc_profile **result)
{
	...
	if (r < 0 && r != SC_ERROR_NOT_SUPPORTED)
		SC_TEST_RET(ctx, r, "Set lifecycle error");
	...
	if (func) {
		...
		sc_debug(ctx, "Unsupported card driver %s", driver);
		sc_profile_free(profile);
		SC_TEST_RET(ctx, SC_ERROR_NOT_SUPPORTED, "Unsupported card driver");
	}
	...
	if ((r = sc_pkcs15init_read_info(card, profile)) < 0) {
		sc_profile_free(profile);
		SC_TEST_RET(ctx, r, "Read info error");
	}
	...
	if ((r = sc_profile_load(profile, profile->name)) < 0
	 ...
	 || (r = sc_profile_finish(profile)) < 0) {
		sc_debug(ctx, "Failed to load profile: %s\n", sc_strerror(r));
		sc_profile_free(profile);
		SC_TEST_RET(ctx, r, "Load profile error");
	}
		...
		struct sc_pkcs15_card *p15card)
{
	...
	for (i = 0; i < nn_objs; i++) {
		...
		r = sc_profile_get_file_by_path(profile, &pininfo->path, &file);
			...
			if (!sc_select_file(p15card->card, &pininfo->path, &file))   {
				...
				sc_debug(ctx, "add '%s' to profile file list", pin_name);
	...
	sc_debug(ctx, "sc_pkcs15init_set_p15card() returns");
}
		...
		struct sc_file *df)
{
	...
	sc_debug(ctx, "sc_pkcs15init_rmdir(%s)\n", sc_print_path(&df->path));
		...
		struct sc_pkcs15init_initargs *args)
{
	...
	if (args->so_pin_len) {
		...
		SC_TEST_RET(ctx, r, "Failed to qualify SO PIN");
		...
		if (pin_obj)   {
			 ...
			 */
			sc_debug(ctx, "Add virtual SO_PIN('%s',flags:%X,reference:%i,path:'%s')", 
					pin_obj->label, pin_info.flags, pin_info.reference, 
					sc_print_path(&pin_info.path));
			r = sc_pkcs15_add_object(p15card, pin_obj);
			SC_TEST_RET(ctx, r, "Failed to add 'SOPIN' AUTH object");
		}
	}
	...
	SC_TEST_RET(ctx, r, "Create 'DIR' error");
			...
			struct sc_pkcs15init_pinargs *args)
{
	...
	/* Now store the PINs */
	sc_debug(ctx, "Store PIN(%s,authID:%s)", pin_obj->label, sc_pkcs15_print_id(&pin_info->auth_id));
	r = sc_pkcs15init_create_pin(p15card, profile, pin_obj, args);
	if (r < 0)
		sc_pkcs15_free_object(pin_obj);
		...
		struct sc_file **ret)
{
	...
	sc_debug(p15card->card->ctx, "create PIN domain (path:%s,ID:%s)", sc_print_path(&df->path), sc_pkcs15_print_id(id));
	...
	if (r >= 0)   {
		sc_debug(p15card->card->ctx, "create PIN DF(path:%s)", sc_print_path(&(*ret)->path));
		...
		unsigned auth_method, unsigned reference)
{
	...
	for (ii=0; ii<nn_objs; ii++)   {
		...
		if (pin_info->auth_method == auth_method && pin_info->reference == reference)
			SC_FUNC_RETURN(ctx, 3, pin_info->reference);
		...
		case SC_PKCS15INIT_SO_PUK:
			...
			break;
		default:
			SC_TEST_RET(ctx, SC_ERROR_INVALID_ARGUMENTS, "Invalid Symbolic PIN reference");
		}
	...
	SC_FUNC_RETURN(ctx, 3, pinfo.reference);
}
...
static int 
select_intrinsic_id(sc_pkcs15_card_t *p15card, struct sc_profile *profile,
		...
		struct sc_path *path)
{
	...
	for (indx = TEMPLATE_INSTANTIATE_MIN_INDEX; indx <= TEMPLATE_INSTANTIATE_MAX_INDEX; indx++)   {
		...
		if (r == SC_ERROR_TEMPLATE_NOT_FOUND)   {
			...
			sc_debug(ctx, "get instance %i of '%s'", indx, t_name);
		...
		int is_new)
{
	...
	SC_TEST_RET(ctx, r, "Failed to encode or update xDF");

	/* Now update the ODF if we have to */
	if (update_odf)
		r = sc_pkcs15init_update_odf(p15card, profile);
	SC_TEST_RET(ctx, r, "Failed to encode or update ODF");
		...
		struct sc_pkcs15_object *object)
{
	...
	if (object == NULL) {
		sc_debug(ctx, "Add nothing; just instantiate this directory file");
	} else if (object->df == NULL) {
		sc_debug(ctx, "Append object");
		object->df = df;
		r = sc_pkcs15_add_object(p15card, object);
		SC_TEST_RET(ctx, r, "Failed to add pkcs15 object");
		object_added = 1;
	} else {
		sc_debug(ctx, "Reuse existing object");
		assert(object->df == df);
	}
		...
		struct sc_pkcs15_object *obj)
{
	...
	if (stored_in_ef) {
		...
		SC_TEST_RET(ctx, r, "Failed to delete object by path");
	} 
		...
		SC_TEST_RET(ctx, r, "Card specific delete object failed");
	} 
		...
		int type, int reference, unsigned char *pinbuf, size_t *pinsize)
{
	...
	if (callbacks.get_key)   {
		...
		if (*pinsize < defsize)
			SC_TEST_RET(ctx, SC_ERROR_BUFFER_TOO_SMALL, "Get transport key error");
		...
		int verify)
{
	...
	sc_debug(ctx, "get and verify PIN(name:%s,type:0x%X,reference:0x%X)\n", ident, type, reference);
	...
	sc_debug(ctx, "get and verify PIN(id:0x%X, type:0x%X, reference:0x%X); p15card:%p\n", pin_id, type, reference, p15card);
	...
	case SC_AC_CHV:
		if (callbacks.get_pin)   {
			...
			sc_debug(ctx, "'get_pin' callback returned %i; pinsize:%i\n", r, *pinsize);
		}
		break;
	default:
...
found: 	/* If it's a PIN, pad it out */
	...
	if (!pin_obj && verify) {
		...
		SC_TEST_RET(ctx, r, "'VERIFY' pin cmd failed");
	}
		...
		struct sc_file *file, int op)
{
	...
	sc_debug(ctx, "r:[0x%08x]\n",r);
	sc_debug(ctx, "acl:[0x%08x]\n",acl);

	for (; r == 0 && acl; acl = acl->next) {
		if (acl->method == SC_AC_NEVER)   {
			SC_TEST_RET(ctx, SC_ERROR_SECURITY_STATUS_NOT_SATISFIED, "Authentication failed: never allowed");
		}
		else if (acl->method == SC_AC_NONE)   {
			sc_debug(ctx, "always allowed");
			break;
		}
		else if (acl->method == SC_AC_UNKNOWN)  {
			sc_debug(ctx, "unknown acl method");
			break;
		}
		sc_debug(ctx, "verify acl(method:%i,reference:%i)\n", acl->method, acl->key_ref);
		...
		struct sc_file *file, struct sc_file **parent)
{
	...
	*parent = NULL;
	...
	if (r == SC_ERROR_FILE_NOT_FOUND && path.len != 2) {
		...
		if (r < 0) {
			sc_debug(ctx, "no profile template for DF %s", sc_print_path(&path));
			SC_FUNC_RETURN(ctx, 3, r);
		}

		r = sc_pkcs15init_create_file(profile, p15card, *parent);
		SC_TEST_RET(ctx, r, "Cannot create parent DF");

		r = sc_select_file(p15card->card, &path, NULL);
		SC_TEST_RET(ctx, r, "Cannot select parent DF");
	} 
		...
		r = sc_profile_get_file_by_path(profile, &path, parent);
		if (r < 0) {
			sc_debug(ctx, "in StarCOS profile there is no template for DF %s", sc_print_path(&path));
			SC_FUNC_RETURN(ctx, 3, r);
		}
	}
	SC_FUNC_RETURN(ctx, 3, r);
		...
		struct sc_file *file)
{
	...
	SC_FUNC_CALLED(ctx, 3);
	sc_debug(ctx, "create file '%s'\n", sc_print_path(&file->path));
	/* Select parent DF and verify PINs/key as necessary */
	r = do_select_parent(profile, p15card, file, &parent);
	SC_TEST_RET(ctx, r, "Cannot create file: select parent error");

	r = sc_pkcs15init_authenticate(profile, p15card, parent, SC_AC_OP_CREATE);
	SC_TEST_RET(ctx, r, "Cannot create file: 'CREATE' authentication failed");

	/* Fix up the file's ACLs */
	r = sc_pkcs15init_fixup_file(profile, p15card, file);
	SC_TEST_RET(ctx, r, "Cannot create file: file fixup failed");

	/* ensure we are in the correct lifecycle */
	r = sc_pkcs15init_set_lifecycle(p15card->card, SC_CARDCTRL_LIFECYCLE_ADMIN);
	if (r != SC_ERROR_NOT_SUPPORTED)
		SC_TEST_RET(ctx, r, "Cannot create file: failed to set lifecycle 'ADMIN'");
		...
		void *data, unsigned int datalen)
{
	...
	if (selected_file->size < datalen) {
		sc_debug(ctx, "File %s too small (require %u, have %u)", 
				sc_print_path(&file->path), datalen, selected_file->size);
		sc_file_free(selected_file);
		SC_TEST_RET(ctx, SC_ERROR_FILE_TOO_SMALL, "Update file failed");
	} 
	...
	for (op = 0; r == 0 && op < SC_MAX_AC_OPS; op++) {
		...
		for (ii = 0; ii < num; ii++) {
			...
			if (acl->key_ref == SC_PKCS15INIT_SO_PIN) {
				...
				sc_debug(ctx, "ACL references unknown symbolic PIN %d", acl->key_ref);
				return SC_ERROR_INVALID_ARGUMENTS;
			}
			...
			if (acl->key_ref == (unsigned int)-1) {
				sc_debug(ctx, "ACL references %s, which is not defined", what);
				return SC_ERROR_INVALID_ARGUMENTS;
			}
