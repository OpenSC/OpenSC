...
int
get_slot_with_card(token_info_t *info)
{
...
int
load_pkcs11_module(token_info_t *info, const char *path_to_pkcs11_library)
{
	CK_RV rv;
	...
	if (C_GetInterface != NULL) {
		/* PKCS#11 3.* C_GetInterface API: Get default interface */
		CK_INTERFACE_PTR interface;
		rv = C_GetInterface(NULL, NULL, &interface, 0);
		if (CKR_OK != rv) {
			fprintf(stderr, "C_GetInterface call failed: 0x%.8lX", rv);
			return 1;
		}
		info->function_pointer = interface->pFunctionList;
	} else {
		CK_FUNCTION_LIST_PTR function_pointer = NULL;
		/* legacy PKCS#11 2.* C_GetFunctionList API */
		CK_RV (*C_GetFunctionList)(CK_FUNCTION_LIST_PTR_PTR) = NULL;
		...
		rv = C_GetFunctionList(&function_pointer);
		if (CKR_OK != rv) {
			fprintf(stderr, "C_GetFunctionList call failed: 0x%.8lX", rv);
			return 1;
		}
		/* backward compatible */
		info->function_pointer = (CK_FUNCTION_LIST_3_2_PTR)function_pointer;
	}
