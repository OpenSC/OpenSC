		...
		test_mech_t *mech, unsigned char **enc_message)
{
	...
	rv = fp->C_EncryptInit(info->session_handle, &enc_mechanism, o->public_handle);
	if (rv != CKR_OK) {
...
openssl_encrypt:
	...
	return encrypt_message_openssl(o, info, message, message_length, mech, enc_message);
}
		...
		test_mech_t *mech, unsigned char **dec_message)
{
	...
	rv = fp->C_DecryptInit(info->session_handle, &dec_mechanism, o->private_handle);
	...
	*dec_message = malloc(dec_message_length);
	...
	rv = fp->C_Decrypt(info->session_handle, enc_message, enc_message_length,
		...
		int multipart)
{
	...
	if (mech->mech != CKM_RSA_X_509 && mech->mech != CKM_RSA_PKCS) {
		debug_print(" [ KEY %s ] Skip encryption for non-supported mechanism %s", o->id_str,
	...
	enc_message_length = encrypt_message(o, info, message, message_length, mech, &enc_message);
	...
	dec_message_length = decrypt_message(o, info, enc_message, enc_message_length, mech, &dec_message);
	free(enc_message);
		...
		unsigned char **sign, int multipart)
{
...
#endif
	rv = fp->C_SignInit(info->session_handle, &sign_mechanism, o->private_handle);
	if (rv == CKR_KEY_TYPE_INCONSISTENT) {
	...
	if (multipart) {
		...
		*sign = malloc(sign_length);
		...
		rv = fp->C_Sign(info->session_handle, message, message_length, *sign, &sign_length);
		if (rv != CKR_OK) {
		...
		test_mech_t *mech, unsigned char *sign, CK_ULONG sign_length)
{
	...
	if (o->type == EVP_PKEY_RSA) {
		...
		if (rv == 1) {
			debug_print(" [  OK %s ] EC Signature of length %lu is valid.", o->id_str,
					message_length);
			mech->result_flags |= FLAGS_SIGN_OPENSSL;
			return 1;
		} else {
		...
		if (rv == 1) {
			debug_print(" [  OK %s ] EdDSA Signature of length %lu is valid.", o->id_str,
					message_length);
			mech->result_flags |= FLAGS_SIGN_OPENSSL;
			EVP_MD_CTX_free(ctx);
			return 1;
		...
		test_mech_t *mech, unsigned char *sign, CK_ULONG sign_length, int multipart)
{
	...
	rv = fp->C_VerifyInit(info->session_handle, &sign_mechanism, o->public_handle);
	if (rv != CKR_OK) {
	...
	if (multipart) {
		...
		/* Second part */
		rv = fp->C_VerifyUpdate(info->session_handle, message + part, message_length - part);
		if (rv != CKR_OK) {
			debug_print("   C_VerifyUpdate: rv = 0x%.8lX", rv);
			goto openssl_verify;
		}
		/* Final */
		rv = fp->C_VerifyFinal(info->session_handle, sign, sign_length);
		name = "C_VerifyFinal";
	} else {
		rv = fp->C_Verify(info->session_handle, message, message_length, sign, sign_length);
		name = "C_Verify";
	}
	if (rv == CKR_OK) {
...
openssl_verify:
	...
	return verify_message_openssl(o, info, message, message_length, mech, sign, sign_length);
}
		...
		int multipart)
{
	...
	if (mech->mech == CKM_RSA_X_509) /* manually add padding */
		message = rsa_x_509_pad_message(const_message, &message_length, o, 0);
	...
	rv = verify_message(o, info, message, message_length, mech, sign, sign_length, multipart);
	free(sign);
	free(message);
	return rv;
}
...
readonly_tests(void **state)
{
	...
	for (i = 0; i < objects.count; i++) {
		...
		if (o->key_type != CKK_RSA && o->key_type != CKK_EC &&
				o->key_type != CKK_EC_EDWARDS && o->key_type != CKK_EC_MONTGOMERY)
			continue;
