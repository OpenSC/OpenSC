...
void
fuzz_get_chunk(sc_reader_t *reader, const uint8_t **chunk, uint16_t *chunk_size)
{
	...
	sc_debug_hex(reader->ctx, SC_LOG_DEBUG_VERBOSE_TOOL, "Returning fuzzing chunk", c, c_size);
...
static int
fuzz_reader_release(sc_reader_t *reader)
{
	if (reader) {
...
static int
fuzz_reader_connect(sc_reader_t *reader)
{
	uint16_t chunk_size;
	const uint8_t *chunk;
...
static int
fuzz_reader_disconnect(sc_reader_t *reader)
{
	return SC_SUCCESS;
}

static int
fuzz_reader_transmit(sc_reader_t *reader, sc_apdu_t *apdu)
{
	const uint8_t *chunk;
	uint16_t chunk_size;
...
void
fuzz_add_reader(struct sc_context *ctx, const uint8_t *Data, size_t Size)
{
	sc_reader_t *reader;
	struct driver_data *data;
	char name[64] = {0};
	...
	snprintf(name, sizeof name - 1, "%zu random byte%s reader (%p)", Size, Size == 1 ? "" : "s", Data);
