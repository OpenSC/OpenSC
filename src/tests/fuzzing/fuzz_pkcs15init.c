...
int
fuzz_profile_load(struct sc_profile *profile, const uint8_t *data, size_t size)
{
	int rv = 0;
...
int
fuzz_get_reader_data(const uint8_t *from, size_t from_size, const uint8_t **to, size_t *to_size)
{
	size_t i = 0;
...
void
do_generate_key(struct sc_profile *profile, struct sc_pkcs15_card *p15card, sc_card_t *card)
{
...
void
do_generate_skey(struct sc_profile *profile, struct sc_pkcs15_card *p15card, sc_card_t *card)
{
...
void
do_store_secret_key(struct sc_profile *profile, struct sc_pkcs15_card *p15card, sc_card_t *card, uint8_t *buf)
{
...
void
do_erase(struct sc_profile *profile, sc_card_t *card)
{
...
LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
	...
	/* First and only sc_pkcs15_bind calling, is omitted in next cases */
	if (!sc_pkcs15_bind(card, NULL, &p15card)) {
