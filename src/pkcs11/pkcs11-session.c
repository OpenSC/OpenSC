...
static CK_RV sc_pkcs11_close_session(CK_SESSION_HANDLE hSession)
{
	...
	if (slot->nsessions == 0 && slot->login_user >= 0) {
		...
		if (sc_pkcs11_conf.atomic)
			...
			if (slot->p11card == NULL || (slot->p11card->flags & SC_PKCS11_CARD_INVALID))
...
{				/* the length of the PIN */
	...
	if (userType == CKU_CONTEXT_SPECIFIC) {
		if (slot->login_user == -1) {
			...
			if (rv == CKR_OK && (slot->p11card && !(slot->p11card->flags & SC_PKCS11_CARD_INVALID))
					&& slot->p11card->framework)
...
CK_RV C_InitPIN(CK_SESSION_HANDLE hSession, CK_CHAR_PTR pPin, CK_ULONG ulPinLen)
{
	...
	if (slot->login_user != CKU_SO) {
		rv = CKR_USER_NOT_LOGGED_IN;
	} else if ((slot->p11card == NULL || (slot->p11card->flags & SC_PKCS11_CARD_INVALID))
			|| slot->p11card->framework->init_pin == NULL) {
