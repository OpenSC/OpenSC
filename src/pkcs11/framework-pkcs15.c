		...
		CK_OBJECT_HANDLE_PTR phObject)
{
	...
	while (ulCount--) {
		...
		case CKA_DECRYPT:
			...
			break;
		case CKA_DERIVE:
			args.usage |= pkcs15_check_bool_cka(attr, SC_PKCS15_PRKEY_USAGE_DERIVE);
			break;
		case CKA_UNWRAP:
...
get_usage_privk(CK_ATTRIBUTE_PTR pTempl, CK_ULONG ulCount, unsigned long *x509_usage, unsigned int *usage)
{
	...
	for (i = 0; i < ulCount; i++) {
		...
		case CKA_WRAP:
		case CKA_ENCRYPT:
			sc_log(context, "invalid type = 0x%0lx", pTempl[i].type);
			...
			CK_MECHANISM_PTR pMechanism)
{
	...
	CK_SIGN_ADDITIONAL_CONTEXT *add_context;
	...
	case CKM_SLH_DSA:
		...
		add_context = (CK_SIGN_ADDITIONAL_CONTEXT *)pMechanism->pParameter;
		switch (add_context->hedgeVariant) {
		...
		/* Context string can not be longer than 255 B */
		if (add_context->ulContextLen > 255) {
			return CKR_MECHANISM_PARAM_INVALID;
		}
		break;
	}
...
register_mechanisms(struct sc_pkcs11_card *p11card)
{
	...
	CK_ULONG aes_min_key_size = ~0, aes_max_key_size = 0;
	CK_ULONG mldsa_min_key_size = ~0, mldsa_max_key_size = 0;
	CK_ULONG mlkem_min_key_size = ~0, mlkem_max_key_size = 0;
	CK_ULONG slhdsa_min_key_size = ~0, slhdsa_max_key_size = 0;
