...
void
print_test_result(const char *algoName, BOOL success, CFErrorRef error, int threadID)
{
...
NSData *
generate_test_data(NSUInteger length)
{
...
NSData *
prepare_input_for_algorithm(SecKeyAlgorithm algo, SecKeyRef key)
{
	...
	if (contains(@"Digest")) {
		if (contains(@"SHA1"))
			return generate_test_data(20);
		if (contains(@"SHA224"))
			return generate_test_data(28);
		if (contains(@"SHA256"))
			return generate_test_data(32);
		if (contains(@"SHA384"))
			return generate_test_data(48);
		if (contains(@"SHA512"))
			return generate_test_data(64);
...
void
run_algorithm_tests(SecKeyRef privateKey, SecKeyRef publicKey, int threadID)
{
...
NSString *
make_safe_filename(NSString *input)
{
...
void
display_identity_details(SecCertificateRef cert, id accessControl)
{
...
BOOL
export_certificate(SecCertificateRef cert, NSString *targetDirectory, BOOL asPEM)
{
...
void
list_objects(BOOL exportCerts, BOOL asPEM)
{
...
test_tokens(int numThreads)
{
	...
	for (NSString *tokenID in tokenIDs) {
		...
		for (NSDictionary *item in keys) {
			...
			if ([keyTypeStr isEqualToString:(__bridge NSString *)kSecAttrKeyTypeRSA])
				humanReadableType = @"RSA";
			if ([keyTypeStr isEqualToString:(__bridge NSString *)kSecAttrKeyTypeECSECPrimeRandom])
				humanReadableType = @"ECC";
