				...
				if ((params = getEC_PARAMS(session, obj, &len))) {
					...
					if (d2i_ASN1_PRINTABLESTRING(&curve, &a, (long)len) != NULL) {
						...
						if (nid != NID_ED25519 &&
								...
								nid != NID_ED448 &&
								nid != NID_X448) {
							util_fatal("Unknown curve OID, expected NID_ED25519 (%d), NID_X25519 (%d), NID_ED448 (%d), NID_X448 (%d), got %d",
									NID_ED25519, NID_X25519,
									NID_ED448, NID_X448, nid);
						}
...
#if defined(EVP_PKEY_ED448)
				else if (type == CKK_EC_EDWARDS && os->length == 57)
