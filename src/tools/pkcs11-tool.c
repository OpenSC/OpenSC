...
int main(int argc, char * argv[])
{
	...
	if (sizeof(options) / sizeof(options[0]) != sizeof(option_help) / sizeof(option_help[0])) {
		util_fatal("Inconsistent size of the options (%zu) and options_help (%zu) structures! "
			   "Please, keep them in sync!",
				sizeof(options)/sizeof(options[0]),
				sizeof(option_help)/sizeof(option_help[0]));
	}
