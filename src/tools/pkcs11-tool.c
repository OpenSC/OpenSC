	...
	CK_OBJECT_HANDLE *hPublicKey, CK_OBJECT_HANDLE *hPrivateKey, const char *type)
{
	...
	if (type != NULL) {
		if (strncmp(type, "RSA:", strlen("RSA:")) == 0 || strncmp(type, "rsa:", strlen("rsa:")) == 0) {
			...
			} else if (strcmp(type, "ML-DSA-44") == 0) {
			...
			if (strlen(type) != strlen("ML-KEM-") + 3 && strlen(type) != strlen("ML-KEM-") + 4) {
				util_fatal("Invalid ML-KEM key type '%s'", type);
			} else if (strcmp(type, "ML-KEM-512") == 0) {
			...
			if (strlen(type) != strlen("SLH-DSA-SHA2-NNNN") && strlen(type) != strlen("SLH-DSA-SHAKE-NNNN") + 7) {
				util_fatal("Invalid SLH-DSA key type '%s'", type);
			} else if (strcmp(type, "SLH-DSA-SHA2-128S") == 0) {
