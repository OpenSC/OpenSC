...
static unsigned char opt_can = 0;
...
main(int argc, char *argv[])
{
	...
	if (opt_change != NULL) {
		pin_change = parse_pin(card, opt_change, "Change", opt_verify == NULL ? &opt_can : NULL);
	...
	if (opt_verify != NULL) {
		pin_verify = parse_pin(card, opt_verify, "Verification", &opt_can);
		if (pin_verify < 0)
			goto out;
	}
