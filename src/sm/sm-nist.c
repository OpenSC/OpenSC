 ...
 * Copyright (C) 2025-2026 Douglas E. Engert <deengert@gmail.com>
 *
...
static const struct sc_asn1_entry c_asn1_nist_cvc_body[C_ASN1_NIST_CVC_BODY_SIZE] = {
		...
		{"DSignatureObject",             SC_ASN1_STRUCT,       SC_ASN1_APP | 0x1F37, 
				SC_ASN1_TAG_SEQUENCE | SC_ASN1_CONS, NULL, NULL},
		{NULL, 0, 0, 0, NULL, NULL}
};
...
nist_sm_verify_certs(struct sc_card *card)
{
	...
	if (priv->params->flags & NIST_SM_FLAGS_SM_IN_CVC_PRESENT) {
		...
		if (!(in_cvc_pkey_ctx = EVP_PKEY_CTX_new_from_name(NIST_LIBCTX, "EC", NULL)) ||
...
sm_nist_open(struct sc_card *card)
{
	...
	if (!(eph_ctx = EVP_PKEY_CTX_new_from_name(NIST_LIBCTX, "EC", NULL)) ||
